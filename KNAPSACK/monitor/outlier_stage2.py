...
pos_output = np.load("intermediate/pinf_list5_log_exp_train_stage2.npy")
neg_output = np.load("intermediate/_pinf_list5_log_exp_train_stage2.npy")

pos = np.load("intermediate/inf_list5_log_exp_train_stage2.npy")
neg = np.load("intermediate/_inf_list5_log_exp_train_stage2.npy")

pos_extra = np.argsort(-1 * np.load("intermediate/pred_ratio_list5_log_exp_train_stage2.npy"), axis = 1)
neg_extra = np.argsort(-1 * np.load("intermediate/_pred_ratio_list5_log_exp_train_stage2.npy"), axis = 1)
...
pos_combined = np.concatenate([pos_output, pos_extra,], axis=1)
neg_combined = np.concatenate([neg_output, neg_extra,], axis=1)
...
pos_output_val = np.load("intermediate/pinf_list5_log_exp_val_stage2.npy")
neg_output_val = np.load("intermediate/_pinf_list5_log_exp_val_stage2.npy")

pos_val = np.load("intermediate/inf_list5_log_exp_val_stage2.npy")
neg_val = np.load("intermediate/_inf_list5_log_exp_val_stage2.npy")

pos_extra_val = np.argsort(-1 * np.load("intermediate/pred_ratio_list5_log_exp_val_stage2.npy"), axis=1)
neg_extra_val = np.argsort(-1 * np.load("intermediate/_pred_ratio_list5_log_exp_val_stage2.npy"), axis=1)

assert pos_val.shape[0] == pos_extra_val.shape[0]
assert neg_val.shape[0] == neg_extra_val.shape[0]

pos_combined_val = np.concatenate([pos_output_val, pos_extra_val], axis=1)
neg_combined_val = np.concatenate([neg_output_val, neg_extra_val], axis=1)
...
best_val_auroc = -1.0
best_ckpt_path = "model/encoder_stage2.pth"
...
for epoch in range(EPOCHS):
    ...
    for s in range(0, len(pairs), pair_batch_size):
        ...
        batch_pair_lbl = torch.tensor(pair_lbls[s : s + pair_batch_size],
...
pos_output = np.load("intermediate/pinf_list5_log_exp_eval_stage2.npy")
neg_output = np.load("intermediate/_pinf_list5_log_exp_eval_stage2.npy")

pos = np.load("intermediate/inf_list5_log_exp_eval_stage2.npy")  
neg = np.load("intermediate/_inf_list5_log_exp_eval_stage2.npy")

pos_extra = np.argsort(-1 * np.load("intermediate/pred_ratio_list5_log_exp_eval_stage2.npy"), axis = 1)
neg_extra = np.argsort(-1 * np.load("intermediate/_pred_ratio_list5_log_exp_eval_stage2.npy"), axis = 1)
...
state_dict = torch.load(
        'model/encoder_stage2.pth', map_location='cuda'
)
